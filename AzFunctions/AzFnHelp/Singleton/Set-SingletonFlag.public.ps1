    ...
    process
    {
        if ($Enable)
        {
            ...
            try{
                ...
                #Get Messages
                # $Msg = $queue.PeekMessagesAsync($PeekMessageCount)
                ...
                if($Messages|Where-Object{$_ -like "Lock-$_RunID-*"})
                {
                    Write-Verbose "True! There was already a lock inplace for this RunID: '$_RunID'"
                    $return = $true
                    return $true
                }
        ...
        elseif ($Disable)
        {
            ...
            # $LockMsg = (Get-FnQueuePeek -Queue $Queue|?{$_.AsString -like "Lock-$RunID*"})
            # Write-Information "Removing singleton lock on queue '$($Queue.Name)' -> '$($LockMsg.AsString)'"
            $Queue|Clear-FnQueue
            # $Queue.DeleteMessageAsync($LockMsg)
        }
    }
    end {
