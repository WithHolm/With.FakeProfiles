...
function Get-RandomFace {
    ...
    begin{
        ...
        {
            Write-Debug "Parent directory does not exsist.. creating"
            new-item -Path $OutputFolder.FullName -ItemType Directory -Force|Out-Null
        }

        Write-verbose "Saving images to  $($OutputFolder.FullName)"
    ...
    process {
        ...
        Write-Debug "Receiving Webcontent and writing to file ($Totalcount calls)"
        while ($runspaces.Processed -contains $false) {
            ...
            foreach ($RS in ($runspaces|?{$_.Status.IsCompleted -and !$_.Processed})) {
                ...
                {
                    Throw "Webcontent is not avalible"
                }
                ...
                Write-Debug "Saving image to $($Testfile.fullname) ($($Completedcount.ToString().PadLeft($Totalcount.tostring().Length,"0"))/$Totalcount)"
